...
#     print(f"Predictions Count: {accuracy_score(y_test, y_pred, normalize=False)}")

#     psymptoms = [Symptom1.get(), Symptom2.get(), Symptom3.get(), Symptom4.get(), Symptom5.get()]

#     # Update symptom indicators
#     for k in range(len(l1)):
#         l2[k] = 1 if l1[k] in psymptoms else 0

#     inputtest = [l2]
#     predicted = gnb.predict(inputtest)[0]

#     # Update text area with the prediction result
#     t3.delete("1.0", END)
...
# # UI Elements
# Label(root, text="E-Doctor", font=("Elephant", 30)).grid(row=1, column=0, columnspan=2, padx=100)

# for i, symptom_label in enumerate(["Symptom 1", "Symptom 2", "Symptom 3", "Symptom 4", "Symptom 5"]):
#     Label(root, text=symptom_label, font=("Elephant", 15)).grid(row=7 + i, column=1, pady=10, sticky=W)

# Button(root, text="Predict", height=2, width=20, command=message, font=("Elephant", 15)).grid(row=15, column=1, pady=20)

# OPTIONS = sorted(l1)
# for i, symptom_var in enumerate([Symptom1, Symptom2, Symptom3, Symptom4, Symptom5]):
#     OptionMenu(root, symptom_var, *OPTIONS).grid(row=7 + i, column=2)
